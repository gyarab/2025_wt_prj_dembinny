...
The project is a focused solution for school-level financial governance, designed around clear separation of concerns and strict multi‑tenancy. It models the lifecycle of a classroom payment: creation, collection, reconciliation and spending. Core domain artifacts include the **PaymentRequest** issued by a treasurer, the recorded **Transaction** representing an incoming payment, and the logged **Expense** representing money leaving the fund. Each class has a linked **BankAccount** and a computed **Fund Balance** derived from confirmed transactions minus published expenses.

Identity and authorization are central. Roles consist of an unauthenticated visitor, a **Student**, a **Parent**, and elevated operators: the **System Admin** and tiered **Treasurer** roles. Treasurers are scoped to a **SchoolClass** via a **ClassMembership**, and per-class permissions are captured in named **FundGroup** entries so administrators can define who may log expenses, create payment requests, or view bank details. Students are represented by **StudentProfile** records and identified for bank reconciliation using a numeric **Variable Symbol (VS)**, generated deterministically per class.

Operational features include bulk provisioning via the **Importer** (CSV import), on‑screen editable previews, and QR generation using the **SPAYD QR** format to simplify payer UX. The reconciliation pipeline anticipates automatic matching against incoming transfers fetched from the **Fio Bank API**, with unmatched items held for manual review.

The architecture enforces the non‑negotiable rule: never query scoped financial entities without filtering by **SchoolClass**, ensuring strict tenant isolation. This design, combined with explicit auditing of imports and notifications, provides a traceable, role-aware system tailored to the practical workflows of teachers, parents and administrators managing class funds.
